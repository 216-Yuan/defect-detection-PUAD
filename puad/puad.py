from typing import Dict, Union, Tuple  # 修复: 导入 Tuple 以支持类型注解
...
class PUAD:
    ...
    def test(self, img: Tensor, return_map: bool = False) -> Union[np.float32, Tuple[np.float32, np.ndarray]]:
        """PUAD 推理：结合可视化异常（EfficientAD）和不可视化异常（马氏距离）
        
        参数:
            img: 输入图像张量
            return_map: 是否返回异常图（用于 PRO 评估）
            
        返回:
            - 默认: 融合后的异常分数 (float)
            - return_map=True: (anomaly_score, anomaly_map)
            
        注意: 当前异常图只包含 EfficientAD 的像素级预测，
             马氏距离是全局特征，无法直接映射到空间位置
        """
        mahalanobis_distance = self.mahalanobis(img)
        
        # 如果需要异常图，则调用 EfficientAD 获取
        if return_map:
            efficient_ad_score, anomaly_map = self.efficient_ad_inference.run(img, return_map=True)
        else:
        ...
        # PUAD 最终分数：可视化异常 + 不可视化异常
        puad_score = normalized_picturable_anomaly_score + normalized_unpicturable_anomaly_score
        
        if return_map:
            # 注意：这里的 anomaly_map 只来自 EfficientAD，因为马氏距离是全局特征
            return puad_score, anomaly_map
        return puad_score
