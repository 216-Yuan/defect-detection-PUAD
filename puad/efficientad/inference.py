...
class EfficientADInference:
    ...
    def run(
        self, img: torch.Tensor, is_for_puad: bool = False, return_map: bool = False
    ) -> Union[np.float64, Tuple[np.float64, torch.Tensor, torch.Tensor], Tuple[np.float64, np.ndarray]]:
        """运行 EfficientAD 推理
        
        参数:
            img: 输入图像张量
            is_for_puad: 是否为 PUAD 训练/推理返回中间特征（teacher/student输出）
            return_map: 是否返回像素级异常图（用于 PRO 评估）
            
        返回:
            - 默认: anomaly_score (float)
            - is_for_puad=True: (anomaly_score, teacher_output, student_output)
        ...
        with torch.no_grad():
            ...
            # 融合 student map 和 autoencoder map 得到最终异常图
